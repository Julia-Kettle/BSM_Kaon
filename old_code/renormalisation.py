            ...
            for ims in range(dimR[2]):
                    for imq in range(dimR[3]):
                        for iboot in range(dimR[4]):
                                ...
                                for ich in range(5):
                                    for jch in range(5):
                                            ren_phys_R[ich,iml,ims,imq,iboot] += T[jch,ich]*ren_R[jch,iml,ims,imq,iboot] #convert to correct basis, T=I in lattice basis
        ...
        for ims in range(dimB[2]):
                for imq in range(dimB[3]):
                    for iboot in range(dimB[4]):
                            ren_B[:,iml,ims,imq,iboot] = (ZA*ZA)*(np.dot(ZschemeB,B[:,iml,ims,imq,iboot]))
                            ren_phys_B[:,iml,ims,imq,iboot] = np.dot(T,ren_B[:,iml,ims,imq,iboot])
                            #for ich in range(5):
                            #    for jch in range(5):
