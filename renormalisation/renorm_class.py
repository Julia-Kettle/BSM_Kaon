...
class Renormalisation:
    ...
    def return_3pts(self):
        ...
        self.matel_3p_2p = np.zeros([len(msea),len(ms),5,self.nboots+1])
        utils.Read_in().read_file_array(filename,self.matel_3p_2p) #3pt/(2pt*2pt) ~ Bag
        self.matel_3p_2p = np.swapaxes(self.matel_3p_2p,0,2)
        self.matel_3p_2p = np.swapaxes(self.matel_3p_2p,1,2)

        filename = 'data/boot_matel_'+latticedim +'cubed_IW_3p.txt'
        self.matel_3p=np.zeros([len(msea),len(ms),5,self.nboots+1])
        utils.Read_in().read_file_array(filename,self.matel_3p) #3pt
        self.matel_3p = np.swapaxes(self.matel_3p,0,2)
        self.matel_3p = np.swapaxes(self.matel_3p,1,2)

        filename = 'data/boot_matel_'+latticedim +'cubed_IW_3p_rat.txt'
        self.matel_3p_rat = np.zeros([len(msea),len(ms),4,self.nboots+1])
        utils.Read_in().read_file_array(filename,self.matel_3p_rat) #3pt1/3pti ~ R
        self.matel_3p_rat = np.swapaxes(self.matel_3p_rat,0,2)
        self.matel_3p_rat = np.swapaxes(self.matel_3p_rat,1,2)

        print "Unrenormalised bag is - ", self.matel_3p_2p[:,:,:,-1]
        print "Unrenormalised bare ratios is - ", self.matel_3p_rat[:,:,:,-1]
        #print "Done" + "\n"
    ...
    def MultRZ(self):
        ...
        self.ren_R = np.swapaxes(self.ren_R,0,-1)
        self.ren_phys_R = np.zeros(dimR)
        self.ren_phys_R = np.swapaxes(self.ren_phys_R,0,-1)
        self.R = np.swapaxes(self.R,0,-1)
        for index in np.ndindex(np.shape(self.R)[:-1]):
            self.ren_R[index] = np.dot(self.R[index],self.bZ_scheme[:,:,-1])/self.bZ_scheme[0][0][-1]
            self.ren_phys_R[index] = np.dot(self.T,self.ren_R[index])
        self.ren_R = np.swapaxes(self.ren_R,0,-1)
        self.ren_phys_R = np.swapaxes(self.ren_phys_R,0,-1)
        self.R = np.swapaxes(self.R,0,-1)
        """
            ...
            for ims in range(dimR[2]):
                    for imq in range(dimR[3]):
                        for iboot in range(dimR[4]):
                                self.ren_R[:,iml,ims,imq,iboot] = (np.dot(self.R[:,iml,ims,imq,iboot],self.bZ_scheme[:,:,-1]))/self.bZ_scheme[0][0][-1]
    ...
    def MultBZ(self):
        ...
        self.ren_B = np.swapaxes(np.zeros(dimB),0,-1)
        self.ren_phys_B = np.swapaxes(np.zeros(dimB),0,-1)
        self.B_phys = np.swapaxes(np.zeros(dimB),0,-1)
        for index in np.ndindex(np.shape(self.B)[:-1]):
            self.ren_B[index] = (self.ZA*self.ZA)*(np.dot(self.bZ_scheme[:,:,-1],self.B[index]))
            self.ren_phys_B[index] = np.dot(self.T,self.ren_B[index])
            for ich in range(1,5):
                self.B_phys[index][ich] = self.ren_phys_B[index][ich]/(self.ZP*self.ZP*self.N[ich])
            self.B_phys[index][0] = self.ren_phys_B[index][0]/(self.ZA*self.ZA*self.N[0])
        self.B = np.swapaxes(self.B,0,-1)
        self.ren_B = np.swapaxes(self.ren_B,0,-1)
        self.ren_phys_B = np.swapaxes(self.ren_phys_B,0,-1)
        self.B_phys = np.swapaxes(self.B_phys,0,-1)
    ...
    def CalculateMatrixElem(self,IB,IR):
        ...
        matel = np.zeros(dimB)
        matel_alt = np.zeros([dimB[0]-1,dimB[1],dimB[2]])
        mk=0.4956
        fk=0.1562
        md=0.003162
        ms=0.08735
