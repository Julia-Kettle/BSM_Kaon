...
class Read_in(Utilities):
    ...
    def store_list_array(self,data,array):
    ...
    def read_file_array(self,filename,array):#nc,n_mseal,n_mval1,n_mval2):
        #read the data in a filelist into an array
        data=self.read_file_list(filename)
        counter = 0
        for index,val in np.ndenumerate(array):
            array[index] = data[counter]
            counter+=1
